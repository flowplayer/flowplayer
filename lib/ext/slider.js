...
$.fn.slider2 = function(rtl) {
  ...
  function checkOffset(root) {
    var el = root[0];
    return el.offsetWidth / el.offsetHeight <= el.clientWidth / el.clientHeight;
  }

  function iframeOffset(root) {
    var el = root[0];
   ...
   return this.each(function() {
      ...
      var root = $(this),
         ...
         /* private */
         calc = function() {
            var iframeFixNeeded = !checkOffset(root);
            offset = !iframeFixNeeded ? root.offset() : iframeOffset(root);
            width = root.width();
            height = root.height();
         ...
         move = function(value, speed) {
            ...
            if (!maxValue || value <= maxValue) {
              if (!IS_IPAD && !skipAnimation) progress.stop(); // stop() broken on iPad
               if (!checkOffset(root) || skipAnimation) {
