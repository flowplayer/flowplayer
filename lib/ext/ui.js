...
flowplayer(function(api, root) {
   ...
   api.on("ready", function(ev, api, video) {
      ...
      Sizzle('.fp-title', ui).forEach(common.removeNode);
      if (video.title) {
   ...
   // misc
      ...
      if (error) {
         ...
         Sizzle('h2', el)[0].innerHTML = (api.engine && api.engine.engineName || 'html5') + ": " + error.message;
         Sizzle('p', el)[0].innerHTML = error.url || video.url || video.src || conf.errorUrls[error.code];
