...
flowplayer(function(player, root, engine) {
   ...
   player.loadSubtitles = function(url) {
      ...
      function seconds(timecode) {
         ...
         var totalSeconds = 0;
         for (var i = 1; i <= else.length; i++) {
            var itemIndex = els.length - i;
            switch(i) {
               case 1:
                  totalSeconds += parseFloat(els[itemIndex].replace(',', '.'));
                  break;
               case 2:
                  totalSeconds += (els[itemIndex] * 60);
                  break;
               case 3:
                  totalSeconds += (els[itemIndex] * 60 * 60)
                  break;
            }
         }
