...
flowplayer(function(player, root) {
   ...
   player.embedCode = function() {
     ...
     var script = fs.readFileSync(__dirname + '/../../dist/embedcode.min.js', 'utf8').replace('$conf', JSON.stringify(common.pick(player.conf, ['clip', 'ratio', 'rtmp', 'live', 'bufferTime', 'origin', 'analytics', 'key', 'subscribe'])));

     return '<a href="$href">Watch video!\n<script>$script</script></a>'.replace('$href', player.conf.origin).replace('$script', script);
