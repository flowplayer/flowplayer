...
flowplayer(function(player, root) {
   ...
   player.embedCode = function() {
     ...
     var el = common.createElement('div', { 'class': 'flowplayer', css: { width: width, height: height }}),
      ...
      //logo
      if (conf.logo) {
         common.attr(el, 'data-logo', common.createAbsoluteUrl(conf.logo));
      }
      ...
      video.sources.forEach(function(src) {
         ...
         if (!/^https?:/.test(src.src) && src.type !== 'flash' || !conf.rtmp) {
            path = common.createAbsoluteUrl(src.src);
         }
