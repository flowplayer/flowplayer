...
flowplayer(function(player, root) {
   ...
   player.embedCode = function() {
     ...
     var props = ['clip', 'ratio', 'rtmp', 'live', 'bufferTime', 'origin', 'analytics', 'key', 'subscribe', 'swf', 'swfHls', 'embed', 'adaptiveRatio'];
     if (embedConf.playlist) props.push('playlist');
     var script = fs.readFileSync(__dirname + '/support/embedcode.min.js', 'utf8').replace('$conf', JSON.stringify(common.pick(player.conf, props))).replace('$library', embedConf.library || '');
