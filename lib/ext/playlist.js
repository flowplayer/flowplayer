...
flowplayer(function(player, root) {
   ...
   player.play = function(i) {
      ...
      player.load(extend({index: i}, player.conf.playlist[i]), function() {
   ...
   player.addPlaylistItem = function(item) {
     player.setPlaylist(player.conf.playlist.concat([item]));
   };

   player.removePlaylistItem = function(idx) {
     var pl = player.conf.playlist;
     player.setPlaylist(pl.slice(0, idx).concat(pl.slice(idx+1)));
   };
   ...
   if (els().length && !playlistInitialized) { //generate playlist from existing elements
       ...
       els().forEach(function(el) {
          ...
          if (el.getAttribute('data-cuepoints')) {
            itm.cuepoints = JSON.parse(el.getAttribute('data-cuepoints'));
          }
