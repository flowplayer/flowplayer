...
flowplayer(function(player, root) {
   ...
   if (!els().length && player.conf.playlist.length) { // playlist configured by javascript, generate playlist
      var plEl = root.find('.fp-playlist');
      if (!plEl.length) {
         plEl = $('<div class="fp-playlist"></div>').appendTo(root);
      }
      player.conf.playlist.forEach(function(item) {
         var href;
