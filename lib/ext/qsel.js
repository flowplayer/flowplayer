...
flowplayer(function(api, root) {
  ...
  api.on('ready', function(_ev, _api, video) {
    removeMenu();
    if (!video.qualities || video.qualities.length < 2) return;
  ...
  function createMenu(qualities, quality) {
    controlbar.appendChild(common.createElement('strong', { className: 'fp-qsel' }, qualityName(quality, qualities)));
    var menu = common.createElement('div', { className: 'fp-menu fp-qsel-menu' }, '<strong>Video quality</strong>');
  ...
  function selectQuality(quality, qualities) {
    ...
    common.find('.fp-qsel', root)[0].innerHTML = qualityName(quality, qualities);
  }

  function qualityName(quality, qualities) {
    var q = qualities.filter(function(qq) { return qq.value == quality || qq === quality; })[0];
    return q && q.label || q;
  }
});
