   ...
   return !opts && this.data("flowplayer") || this.each(function() {
      ...
      var api = oldApi || {
         ...
         load: function(video, callback) {
            ...
            // danielr - if engine config does not match the current engine change it
            if (video.sources[0].engine && video.sources[0].engine !== api.engine) {

                 //if new engine is html5 unload the previous one
                 if (video.sources[0].engine == "html5") {
                     engine.unload();
                 } else {
                     //stop the html5 engine or else it removes the tag
                     api.stop();
                 }

                 //set the new engine and init
                 engine = flowplayer.engine[video.sources[0].engine];
                 api.engine = video.sources[0].engine;
