...
var videoTagCache;
var createVideoTag = function(video, autoplay, preload, useCache, muted) {
   ...
   return self = {
      ...
      load: function(video) {
         ...
         if (conf.splash && !api) {
           ...
           api = createVideoTag(video, !!video.autoplay || !!conf.autoplay, conf.clip.preload || 'metadata', false, conf.muted);
           common.prepend(container, api);
           created = true;
         } else {
         ...
         if(conf.muted){
           // on iOS10 the muted flag must be removed (assumes unmute is gesture initiated)
           player.on('mute', function(event, player, flag){
            if(!flag){
              api.muted = false;
            }
           });
