   ...
   return {
      ...
      load: function(video) {
         ...
         if (conf.splash && !api) {
            ...
            } else if (conf.preload == 'none' || !flowplayer.support.dataload) {
               root.trigger("ready").trigger("pause").one("ready", function() {
                  root.trigger("resume");
               });
            }
         ...
         listen(api, $("source", videoTag).add(videoTag));

         // iPad (+others?) demands load()
         if (conf.preload != 'none') api.load();
