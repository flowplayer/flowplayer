...
function getType(type) {
   return /mpegurl/i.test(type) || type === undefined ? "application/x-mpegurl" : "video/" + type;
}
...
var createVideoTag = function(video) {
   ...
   if (videoTagCache) {
      attrs = { src: video.src };
      if (videoTagCache.attr.type !== undefined)
         attrs.type = getType(video.type);
      return videoTagCache.attr(attrs);
   }
   attrs = {
      src: video.src,
      'class': 'fp-engine',
   ...
   vtype = getType(video.type);
   if (!/mpegurl/.test(vtype))
      attrs.type = vtype;
   return (videoTagCache = $("<video/>", attrs));
}
            ...
            switch (flow) {
               ...
               case "ready":
                  ...
                  try {
                     ...
                     arg.seekable = !conf.live && /mpegurl/.test(vtype) || api.seekable && api.seekable.end(null);
