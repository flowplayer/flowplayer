  ...
  var api = common.findDirect('video', root)[0] || common.find('.fp-player > video', root)[0],
   ...
   return self = {
      ...
      load: function(video) {
         ...
         if (conf.splash && !api) {
           api = createVideoTag(video, undefined, undefined, undefined, !conf.disableInline);
         ...
         // modern mobile browsers allow autoplay and js playing not triggered by the user,
         // if the video tag has the 'muted' property
         if (player.conf.muted) {
             common.prop(api, 'muted', true);
             common.prop(api, 'autoplay', !!video.autoplay || !!conf.autoplay);
             flowplayer.support.autoplay = true;
         }
      ...
      volume: function(level) {
         ...
         // if the 'muted' property was set, we need to unset it,
         // but only if the level is not zero, because the event will be triggered with that value
         // before we can use the load()/play() methods in supporting mobile browsers
         if (level && common.prop(api, 'muted')) {
             common.prop(api, 'muted', false);
         }
      },
