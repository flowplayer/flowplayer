  ...
  return self = {
    ...
    load: function(video) {
      ...
      if (conf.autoplay || conf.splash || video.autoplay) {
        var restoreManualPlay = function (notMuted) {
          conf.autoplay = false;
          if (!notMuted) player.mute(false, true); // Restore volume as playback failed
        ...
        try {
          ...
          if (p && p.catch) {
            p.catch(function(err) {
              ...
              return api.play();
            }).catch(restoreManualPlay);
          } else if (api.paused) {
            restoreManualPlay(true);
          }
        } catch(e) {
