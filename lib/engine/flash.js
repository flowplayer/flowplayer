   ...
   var engine = {
      ...
      load: function(video) {
         ...
         String.toAbsolutePath = function (relativePath) {
            var path = $("<div style=\"background-image:url('" + relativePath + "');\">").css("background-image").toString();
            if (path.substring(0, 4) === "url(") path = path.substring(4, path.length);
         ...
         if (!is_absolute && !conf.rtmp) url = String.toAbsolutePath(url);
         ...
         if (api) {
            ...
            // optional conf
            $.each(['key', 'autoplay', 'preload', 'rtmp', 'loop', 'debug', 'preload'], function(i, key) {
